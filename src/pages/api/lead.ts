...
function buildEmailHtml(data: {
  ...
  const pathMatch = pagePath.match(/\/lawn-care\/([^/]+)/);
  ...
  const sourceLabel = isHomePage ? 'Homepage' : `${cityDisplay} — Lawn Care`;
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f7f9f8;border:1px solid #d8e2dc;border-top:none;border-radius:0 0 12px 12px;">
          <tr>
            <td style="padding:16px 32px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="font-size:12px;color:#748c7e;">🕐 ${timestamp}</td>
                  <td style="text-align:right;font-size:12px;color:#748c7e;">🌐 ${pagePath}</td>
                </tr>
              </table>
            </td>
          </tr>
...
export async function POST({ request }: { request: Request }) {
  ...
  const { site_name, first_name, last_name, phone, email, comment, page_path } = body;
  ...
  const html = buildEmailHtml({
    siteName: site_name || 'NKY Lawn Care Pros',
    fullName,
    email,
    phone: phone || '',
    comment: comment || '',
  ...
  try {
    await resend.emails.send({
      from: 'NKY Lawn Care Alerts <alerts@mursenmaintenance.com>',
      to: [process.env.NOTIFY_EMAIL || 'hello@scalesolving.com'],
